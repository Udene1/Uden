...
export async function inspectGitWorkspace(cwd: string): Promise<GitWorkspaceState> {
  ...
  return {
    ...
    ahead,
    behind,
    clean: parsed.changed === 0 && parsed.staged === 0,
...
export async function checkoutGitBranch(cwd: string, branch: string): Promise<LocalCommandResult> {
  ...
  if (!normalizedBranch || normalizedBranch.startsWith('-') || normalizedBranch.includes('..')) {
    throw new Error('Invalid Git branch name.');
  }

  // Use `git switch -- <branch>` rather than `git checkout -- <branch>`:
  // the latter is path-oriented syntax and can silently fail to express a branch switch.
  const request = {
    workspaceRoot: cwd,
    cwd,
    program: 'git',
    args: ['switch', '--', normalizedBranch],
  };
