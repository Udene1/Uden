...
export default function DashboardOverview() {
  ...
  async function loadWorkspace() {
    ...
    try {
      const [tasksResult, usageResult, graphResult] = await Promise.all([
        api.getTasks(apiKey),
      ...
      if (first?.id) {
        const detail = await getGraph(first.id);
        setLatestGraph(detail.graph);
      } else {
        setLatestGraph(null);
      }
      setError(null);
    ...
    <div className="space-y-7 pb-10">
      <section className="pt-2">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-[var(--text-secondary)] max-w-2xl">Give Uden the outcome you want. It plans the work, routes each step, executes it, verifies the result, and keeps the durable work trail.</p>
          </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          <div key={label} className="glass-card p-4 border border-[var(--border-color)]">
            <div className="flex items-center gap-2 text-xs text-[var(--text-secondary)]"><Icon size={14} /> {label}</div>
            {loading ? <CardSkeleton /> : <div className="mt-2 text-xl font-semibold text-[var(--text-primary)]">{value}</div>}
          </div>
        ))}
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <div><h2 className="text-lg font-semibold text-[var(--text-primary)]">Start with an outcome</h2><p className="text-sm text-[var(--text-secondary)] mt-1">These start real work; they are not simulated examples.</p></div>
        </div>
      ...
      <section className="grid lg:grid-cols-[1.5fr_1fr] gap-6">
        <div className="glass-card p-5 md:p-6 border border-[var(--border-color)]">
          <div className="flex items-center justify-between mb-5">
            <div><h2 className="text-lg font-semibold text-[var(--text-primary)] flex items-center gap-2"><Workflow size={18} /> Latest execution</h2><p className="text-sm text-[var(--text-secondary)] mt-1">The most recent durable graph recorded by Uden.</p></div>
            <Link href="/graphs" className="text-sm text-[var(--accent-primary)] flex items-center gap-1">Open graphs <ArrowRight size={14} /></Link>
          </div>
          {loading ? <div className="space-y-3"><CardSkeleton /><CardSkeleton /><CardSkeleton /></div> : latestGraph ? (
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="min-w-0"><p className="font-medium text-[var(--text-primary)] truncate">{latestGraph.goal || latestGraph.id}</p><p className="font-mono text-xs text-[var(--text-muted)] mt-1">{latestGraph.id}</p></div>
                <StatusBadge status={latestGraph.status || 'unknown'} />
              </div>
              <div className="grid grid-cols-3 gap-2">
                <Metric label="Nodes" value={String(graphStats.nodes)} />
                <Metric label="Completed" value={String(graphStats.completed)} />
                <Metric label="Running" value={String(graphStats.running)} />
              </div>
              <div className="flex flex-wrap gap-2 pt-1">
                {(latestGraph.nodes || []).slice(0, 8).map((node) => <span key={node.id} className="px-3 py-1.5 rounded-full border border-[var(--border-color)] text-xs text-[var(--text-secondary)]">{node.title} · {node.status}</span>)}
              </div>
            </div>
          ) : <div className="py-10 text-center text-sm text-[var(--text-secondary)]"><Workflow className="mx-auto mb-3 opacity-50" /><p>No execution graph has been recorded yet.</p></div>}
        </div>

        <div className="glass-card p-5 md:p-6 border border-[var(--border-color)]">
          <div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-semibold text-[var(--text-primary)]">Usage</h2><p className="text-sm text-[var(--text-secondary)] mt-1">Recorded workspace consumption.</p></div><Link href="/analytics" className="text-sm text-[var(--accent-primary)]">Details</Link></div>
          {loading ? <CardSkeleton /> : usage ? <div className="space-y-5">
            <div><div className="flex justify-between text-sm mb-2"><span className="text-[var(--text-secondary)]">Monthly budget</span><span className="text-[var(--text-primary)]">{usage.budgetUsedPercent.toFixed(1)}%</span></div><div className="h-2 rounded-full bg-[var(--bg-secondary)] overflow-hidden"><div className="h-full bg-[var(--accent-primary)] rounded-full transition-all" style={{ width: `${Math.min(100, Math.max(0, usage.budgetUsedPercent))}%` }} /></div></div>
            <div className="grid grid-cols-2 gap-4"><Metric label="Input tokens" value={formatNumber(usage.totalTokensIn)} /><Metric label="Output tokens" value={formatNumber(usage.totalTokensOut)} /><Metric label="Escalations" value={formatNumber(usage.escalationCount)} /><Metric label="Routing savings" value={formatCurrency(usage.savingsEstimateCents)} /></div>
          </div> : <p className="text-sm text-[var(--text-secondary)]">No usage data recorded yet.</p>}
        </div>
      </section>
          ...
          <div className="space-y-2">
            {recentTasks.map((task) => (
              <Link key={task.id} href={`/tasks/${task.id}`} className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between rounded-xl border border-[var(--border-color)] p-4 hover:bg-[var(--bg-hover)] transition-colors">
                ...
                <div className="flex items-center gap-4 shrink-0"><StatusBadge status={task.status} />{task.modelUsed && <span className="text-xs text-[var(--text-muted)]">{task.modelUsed}</span>}{task.status === 'completed' && <span className="text-xs text-[var(--accent-primary)]">Verified</span>}</div>
              </Link>
            ))}
          </div>
        )}
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-secondary)] p-3"><div className="text-xs text-[var(--text-muted)]">{label}</div><div className="text-sm font-semibold mt-1 text-[var(--text-primary)]">{value}</div></div>;
}
