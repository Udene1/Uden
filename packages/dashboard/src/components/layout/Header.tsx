...
import { Command, LogOut, Moon, Search, Sun, Plus } from 'lucide-react';
...
export default function Header() {
  const pathname = usePathname(); const { data: session } = useSession(); const { theme, setTheme } = useTheme(); const [mounted,setMounted]=useState(false);
  ...
  return <header className="h-[64px] md:h-[68px] shrink-0 flex items-center justify-between px-3 md:px-6 m-2 md:m-4 rounded-2xl border bg-[var(--bg-card)]/85 backdrop-blur-xl shadow-sm z-10" style={{borderColor:'var(--border-color')}}>
