...
const titles: Record<string, string> = {
  dashboard: 'Workspace', tasks: 'Tasks', graphs: 'Execution graphs', projects: 'Projects', analytics: 'Analytics', settings: 'Settings',
};
...
export default function Header() {
  ...
  useEffect(() => {
    const open = () => window.dispatchEvent(new Event('uden:command-palette'));
    const listener = () => open();
    window.addEventListener('uden:command', listener);
    return () => window.removeEventListener('uden:command', listener);
  }, []);
      ...
      <header className="z-10 m-2 flex h-[64px] shrink-0 items-center justify-between rounded-2xl border bg-[var(--bg-card)]/85 px-3 shadow-sm backdrop-blur-xl md:m-4 md:h-[68px] md:px-6" style={{ borderColor: 'var(--border-color)' }}>
        <div className="flex min-w-0 items-center gap-2.5 md:gap-3">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-base font-semibold text-[var(--text-primary)] md:text-lg">{title}</h1>
            {segment === 'dashboard' && <p className="mt-0.5 hidden text-xs text-[var(--text-muted)] sm:block">Plan, execute, verify.</p>}
          </div>
        </div>
