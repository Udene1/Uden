...
class ApiClient {
 ...
 async registerTenant(name:string,email:string,password:string):Promise<{session_token:string;tenant:Tenant}>{return this.fetcher('/auth/register',{method:'POST',body:JSON.stringify({name,email,password})});}
 async login(email:string,password:string):Promise<{session_token:string;tenant:Tenant}>{return this.fetcher('/auth/login',{method:'POST',body:JSON.stringify({email,password})});}
