...
export interface GraphAnalytics { totals:{graphs:number;completed:number;failed:number;escalationAttempts:number}; byDomain:Array<{domain:string;nodes:number;costCents:number;avgQuality:number;completed:number;failed:number}>; byModel:Array<{model:string;attempts:number;costCents:number;avgQuality:number;failures:number}>; recentGraphs:Array<{id:string;status:string;goal:string;costCents:number;tokensIn:number;tokensOut:number;avgQuality:number;nodes:number;blocked:number}>; }
