...
export async function createTenant(db: D1Database, tenant: Tenant) {
  await db.prepare(`
    INSERT INTO tenants (
      id, name, email, api_key_hash, quality_preference, monthly_budget_cents, mode, bring_own_keys, provider_keys_json
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    tenant.id,
    tenant.name,
    tenant.email || '',
    tenant.apiKeyHash,
    tenant.qualityPreference || 'balanced',
    tenant.monthlyBudgetCents || 10000,
    tenant.defaultMode || 'permissionless',
    tenant.bringOwnKeys ? 1 : 0,
    tenant.providerKeys ? JSON.stringify(tenant.providerKeys) : null
  ).run();
}
...
export async function updateTenant(db: D1Database, id: string, updates: Partial<Tenant>) {
  const keyMap: Record<string, string> = {
    ...
    providerKeys: 'provider_keys_json'
  };
  ...
  const values = keys.map(k => {
    ...
    if (k === 'bringOwnKeys') return val ? 1 : 0;
    if (k === 'providerKeys') return val ? JSON.stringify(val) : null;
    return val;
  });
...
function mapTenantRow(row: any): Tenant {
  let providerKeys;
  if (row.provider_keys_json) {
    try {
      providerKeys = JSON.parse(row.provider_keys_json);
    } catch {
  ...
  return {
    ...
    qualityPreference: row.quality_preference || 'balanced',
    monthlyBudgetCents: row.monthly_budget_cents ?? 10000,
    defaultMode: row.mode || 'permissionless',
...
export async function createProject(db: D1Database, project: Project) {
  await db.prepare(`
    INSERT INTO projects (id, tenant_id, name, description, context) VALUES (?, ?, ?, ?, ?)
...
export async function getProjects(db: D1Database, tenantId: string): Promise<Project[]> {
  const result = await db.prepare(`SELECT * FROM projects WHERE tenant_id = ? ORDER BY created_at DESC`).bind(tenantId).all<any>();
...
export async function getProjectById(db: D1Database, id: string, tenantId: string): Promise<Project | null> {
  const row = await db.prepare(`SELECT * FROM projects WHERE id = ? AND tenant_id = ?`).bind(id, tenantId).first<any>();
  if (!row) return null;
  return {
    id: row.id,
    tenantId: row.tenant_id,
...
export async function createTask(db: D1Database, task: Task, routingPlan?: RoutingPlan) {
  await db.prepare(`
    INSERT INTO tasks (
      id, tenant_id, project_id, prompt, mode, status,
      classified_tier, classified_domain, classified_complexity, expected_format,
      model_used, output, proposal_json, routing_plan_json, quality_score,
      total_cost_cents, tokens_in, tokens_out, escalation_count
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    task.id,
    task.tenantId,
    ...
    task.mode,
    task.status,
    task.classifiedTier || null,
    task.classifiedDomain || null,
    task.classifiedComplexity || null,
    task.expectedFormat || 'markdown',
    task.modelUsed || null,
    task.output || null,
    task.proposal ? JSON.stringify(task.proposal) : null,
    routingPlan ? JSON.stringify(routingPlan) : null,
...
export async function getTask(db: D1Database, id: string, tenantId: string): Promise<(Task & { routingPlan?: RoutingPlan }) | null> {
...
export async function updateTask(db: D1Database, id: string, tenantId: string, updates: Partial<Task> & { routingPlan?: RoutingPlan }) {
  const keyMap: Record<string, string> = {
    ...
    classifiedComplexity: 'classified_complexity',
    expectedFormat: 'expected_format',
    modelUsed: 'model_used',
    output: 'output',
    proposal: 'proposal_json',
    routingPlan: 'routing_plan_json',
...
function mapTaskRow(row: any): Task & { routingPlan?: RoutingPlan } {
  ...
  let routingPlan: RoutingPlan | undefined;
  if (row.routing_plan_json) {
    try {
      routingPlan = JSON.parse(row.routing_plan_json);
    } catch {
  ...
  return {
    ...
    prompt: row.prompt,
    mode: row.mode || 'permissionless',
    status: row.status,
    classifiedTier: row.classified_tier || undefined,
    classifiedDomain: row.classified_domain || undefined,
    classifiedComplexity: row.classified_complexity || undefined,
    expectedFormat: row.expected_format || 'markdown',
...
export async function createEscalationLog(db: D1Database, log: EscalationLog) {
  await db.prepare(`
    INSERT INTO escalation_logs (id, task_id, from_model, to_model, reason, quality_score, attempt_number)
    VALUES (?, ?, ?, ?, ?, ?, ?)
...
export async function getUsageSummary(db: D1Database, tenantId: string) {
  ...
  const modelCosts = await db.prepare(`
    SELECT model, SUM(cost_cents) as cost FROM usage_records WHERE tenant_id = ? GROUP BY model
  `).bind(tenantId).all<any>();

  const providerCosts = await db.prepare(`
    SELECT provider, SUM(cost_cents) as cost FROM usage_records WHERE tenant_id = ? GROUP BY provider
  `).bind(tenantId).all<any>();

  const costByModel: Record<string, number> = {};
  for (const row of modelCosts.results || []) {
    costByModel[row.model] = row.cost;
  }

  const costByProvider: Record<string, number> = {};
  for (const row of providerCosts.results || []) {
    costByProvider[row.provider] = row.cost;
  }

  const totalCost = totals?.total_cost || 0;
  const totalTasks = taskStats?.total_tasks || 0;
  const escalationCount = taskStats?.total_escalations || 0;

  return {
    totalCostCents: Math.round(totalCost * 100) / 100,
    totalTokensIn: totals?.total_tokens_in || 0,
    totalTokensOut: totals?.total_tokens_out || 0,
    totalTasks,
    completedTasks: taskStats?.completed_tasks || 0,
    failedTasks: taskStats?.failed_tasks || 0,
    escalationCount,
    escalationRate: totalTasks > 0 ? Math.round((escalationCount / totalTasks) * 100) / 100 : 0,
    averageQualityScore: Math.round((taskStats?.avg_quality || 0) * 10) / 10,
    costByModel,
    costByProvider,
