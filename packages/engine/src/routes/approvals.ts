...
approvalRoutes.post('/graphs/:graphId/nodes/:nodeId/request', async c => {
  ...
  try {
    ...
    const graph = await requestGraphNodeApproval(c.env.DB, tenantId, c.req.param('graphId'), c.req.param('nodeId'), typeof body.reason === 'string' ? body.reason : undefined, c.get('executionPrincipal'));
...
approvalRoutes.post('/graphs/:graphId/nodes/:nodeId/approve', async c => {
  ...
  try {
    ...
    const body = await c.req.json().catch(() => ({}));
    const approvedBy = typeof body.approvedBy === 'string' ? body.approvedBy : c.get('executionPrincipal');
...
approvalRoutes.post('/graphs/:graphId/nodes/:nodeId/reject', async c => {
  ...
  try {
    await requirePermission(c.env.DB, tenantId, 'graph:resume');
    const body = await c.req.json().catch(() => ({}));
    const rejectedBy = typeof body.rejectedBy === 'string' ? body.rejectedBy : c.get('executionPrincipal');
...
approvalRoutes.post('/repairs/:proposalId/approve', async c => { const tenantId=c.get('tenantId'); try { await requirePermission(c.env.DB,tenantId,'graph:resume'); const body=await c.req.json().catch(()=>({})); const approvedBy=typeof body.approvedBy==='string'?body.approvedBy:c.get('executionPrincipal'); const proposal=await approveGraphRepair(c.env,tenantId,c.req.param('proposalId'),approvedBy); await writeAudit(c.env.DB,tenantId,'graph.node.repair.approved','graph_repair_proposal',proposal.id,undefined,c.get('requestId'),{graphId:proposal.graphId,nodeId:proposal.nodeId,attemptNumber:proposal.attemptNumber,approvedBy}); return c.json({proposal},200); } catch(error) { const message=sanitizeError(error); return c.json({error:message},message==='Forbidden'?403:409); } });
approvalRoutes.post('/repairs/:proposalId/reject', async c => { const tenantId=c.get('tenantId'); try { await requirePermission(c.env.DB,tenantId,'graph:resume'); const body=await c.req.json().catch(()=>({})); const rejectedBy=typeof body.rejectedBy==='string'?body.rejectedBy:c.get('executionPrincipal'); const proposal=await rejectGraphRepair(c.env.DB,tenantId,c.req.param('proposalId'),rejectedBy,typeof body.reason==='string'?body.reason:undefined); await writeAudit(c.env.DB,tenantId,'graph.node.repair.rejected','graph_repair_proposal',proposal.id,undefined,c.get('requestId'),{graphId:proposal.graphId,nodeId:proposal.nodeId,attemptNumber:proposal.attemptNumber,rejectedBy}); return c.json({proposal},200); } catch(error) { const message=sanitizeError(error); return c.json({error:message},message==='Forbidden'?403:409); } });
