...
export async function prepareBackgroundGraph(env: HonoEnv['Bindings'], tenantId: string, plan: TaskGraphPlan, projectId?: string): Promise<{ graphId: string }> {
  ...
  const graph: TaskGraph = {
    id: graphId, rootTaskId: rootTask.id, goal: plan.goal,
  ...
  // Persist before enqueueing. The queue message carries the durable graph ID,
  // so redelivery resumes the same graph instead of creating a duplicate.
  await persistGraph(env.DB, tenantId, graph, projectId);
  return { graphId };
}

export function backgroundQueueMessage(tenantId: string, graphId: string): GraphExecutionQueueMessage {
  return { tenantId, graphId, enqueuedAt: new Date().toISOString() };
}
