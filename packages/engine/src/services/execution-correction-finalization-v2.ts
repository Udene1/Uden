...
export async function resolveValidatedExecutionContradiction(db:D1Database,input:{tenantId:string;graphId:string;contradictionId:string;validationId:string;resolutionReason:string}):Promise<void>{
  ...
  const result=await db.batch([
    db.prepare(`INSERT INTO execution_contradiction_resolutions
      (id,tenant_id,graph_id,contradiction_id,validation_id,plan_revision_id,corrective_action_id,executor_principal,validator_principal,resolution_reason)
    ...
    ).bind(resolutionId,reason,input.validationId,input.tenantId,input.graphId,input.contradictionId),
