...
import { persistGraph, acquireGraphExecutionLease, recordGraphAttempt } from './graph-persistence';
import { registerExecutionRuntime, authorizeRuntimeExecution, markRuntimeExecutionInFlight, completeRuntimeExecution, claimRecoverableRuntimeExecution } from './execution-runtimes';
...
describe.sequential('execution runtime fencing D1 integration', () => {
  ...
  beforeAll(async () => {
    ...
    await db.prepare(`INSERT INTO tasks (id,tenant_id,prompt,status) VALUES (?,?,?,?)`).bind('runtime-recovery-root','runtime-tenant','runtime recovery test','processing').run();
  });
  ...
  it('requires the runtime to advertise the requested capability', async () => {
    ...
    await persistGraph(db, 'runtime-tenant', graph); const version = await acquireGraphExecutionLease(db, 'runtime-tenant', graph.id, 'runtime-worker');
    await expect(authorizeRuntimeExecution(db, 'runtime-tenant', { runtimeId: 'desktop-a', graphId: graph.id, nodeId: 'node', attemptId: 'runtime-capability-attempt', executionOwner: 'runtime-worker', executionVersion: version!, leaseExpiresAt: new Date(Date.now()+60000).toISOString(), capability: 'command.exec', command: 'echo', args: ['hello'] })).rejects.toThrow('lacks the requested capability');
  });
  it('fences runtime completion to the exact graph owner and generation', async () => {
    ...
    await persistGraph(db, 'runtime-tenant', graph); const version = await acquireGraphExecutionLease(db, 'runtime-tenant', graph.id, 'runtime-worker-a');
    const request = { runtimeId: 'desktop-b', graphId: graph.id, nodeId: 'node', attemptId: 'runtime-fence-attempt', executionOwner: 'runtime-worker-a', executionVersion: version!, leaseExpiresAt: new Date(Date.now()+60000).toISOString(), capability: 'command.exec' as const, command: 'echo', args: ['hello'] };
    await authorizeRuntimeExecution(db, 'runtime-tenant', request); await markRuntimeExecutionInFlight(db, 'runtime-tenant', request);
    ...
    const row = await db.prepare(`SELECT status FROM runtime_executions WHERE tenant_id=? AND attempt_id=?`).bind('runtime-tenant','runtime-fence-attempt').first<{status:string}>(); expect(row?.status).toBe('in_flight');
  });
  it('allows only one recovery worker to claim an unresolved runtime execution', async () => {
    await registerExecutionRuntime(db, 'runtime-tenant', { id: 'desktop-recovery', tenantId: 'runtime-tenant', kind: 'desktop_local', state: 'online', capabilities: ['command.exec'], lastHeartbeatAt: new Date().toISOString() });
    const graph: TaskGraph = { id: 'runtime-recovery-graph', rootTaskId: 'runtime-recovery-root', goal: 'runtime recovery', createdAt: new Date().toISOString(), nodes: [{ id: 'node', title: 'Node', prompt: 'recover', domain: 'general', complexity: 1, expectedFormat: 'text', recommendedTier: 1, dependencies: [], contextFrom: [], status: 'ready', attemptedModels: [] }] };
    await persistGraph(db, 'runtime-tenant', graph); const version = await acquireGraphExecutionLease(db, 'runtime-tenant', graph.id, 'runtime-worker');
    const request = { runtimeId:'desktop-recovery', graphId:graph.id, nodeId:'node', attemptId:'runtime-recovery-attempt', executionOwner:'runtime-worker', executionVersion:version!, leaseExpiresAt:new Date(Date.now()+60000).toISOString(), capability:'command.exec' as const };
    await authorizeRuntimeExecution(db,'runtime-tenant',request); await markRuntimeExecutionInFlight(db,'runtime-tenant',request);
    const first = await claimRecoverableRuntimeExecution(db,'runtime-tenant','runtime-recovery-attempt','recovery-worker-a'); expect(first?.attemptId).toBe('runtime-recovery-attempt');
    const second = await claimRecoverableRuntimeExecution(db,'runtime-tenant','runtime-recovery-attempt','recovery-worker-b'); expect(second).toBeNull();
  });
});
