...
export async function getGitHubPullDiff(env: Env, tenantId: string, owner: string, repo: string, number: number): Promise<string> { const token = await getToken(env, tenantId); if (!Number.isInteger(number) || number < 1) throw new Error('Invalid pull request number'); const headers = new Headers({ Accept: 'application/vnd.github.diff' }); const response = await fetch(`${API_URL}${repoPath(owner, repo, `/pulls/${number}`)}`, { headers: { Authorization: `Bearer ${token}`, 'User-Agent': 'Uden/1.0', Accept: 'application/vnd.github.diff' } }); if (!response.ok) throw new Error(`GitHub pull request diff failed (${response.status})`); return (await response.text()).slice(0, MAX_READ);
}
