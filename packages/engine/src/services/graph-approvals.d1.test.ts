...
describe('graph approvals D1 integration', () => {
  ...
  beforeAll(async () => { const platform = await getPlatformProxy({ configPath: resolve(root, 'wrangler.test.jsonc'), persist: false }); db = platform.env.DB as D1Database; dispose = platform.dispose; await execSqlFile(db, resolve(here, '../db/schema.sql')); for (const migration of ['0002_task_graph_persistence.sql','0003_graph_durable_execution.sql','0010_graph_node_approvals.sql','0011_runtime_job_linkage.sql','0012_graph_node_tool_persistence.sql','0013_graph_verification_repair.sql','0016_execution_principal.sql','0017_durable_graph_approvals.sql']) await execSqlFile(db, resolve(root, `migrations/${migration}`)); await db.prepare('INSERT INTO tenants (id,name,email,api_key_hash) VALUES (?,?,?,?)').bind('approval-tenant','Approval Tenant','approval@example.test','approval-hash').run(); });
