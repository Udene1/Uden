...
export function validateTaskGraphPlan(plan: TaskGraphPlan): void {
  ...
  for (const node of plan.nodes) {
    for (const dependency of node.dependencies) {
      if (!ids.has(dependency)) throw new Error(`Invalid graph: node '${node.id}' depends on missing node '${dependency}'`);
    }
  }
...
async function executeGraphState(
  env: HonoEnv['Bindings'], tenantId: string, graph: TaskGraph,
  qualityPreference: 'cost-optimized' | 'balanced' | 'quality-first', owner: string,
  ...
  const persist = async (status = 'running', error?: string) => {
    ...
    await persistGraphSnapshot(env.DB, tenantId, graph, status, active, error, { owner });
  };
...
export async function resumeTaskGraph(env: HonoEnv['Bindings'], tenantId: string, graphId: string): Promise<GraphExecutionResult> {
  ...
  const qualityPreference = tenant?.qualityPreference || 'balanced';
  for (const node of graph.nodes) {
    if (node.status === 'running') { node.status = 'ready'; node.error = 'Recovered after interrupted execution; retrying node'; }
  }
  blockDependents(graph);
