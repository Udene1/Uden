...
describe('graph persistence D1 integration', () => {
  ...
  it('prevents concurrent graph ownership',async()=>{const leaseGraph: TaskGraph = { ...graph, id:'lease-graph', rootTaskId:'root-lease', goal:'lease graph' };await persistGraph(db,'tenant-a',leaseGraph,undefined,{owner:'owner-initial'});const first=await acquireGraphExecutionLease(db,'tenant-a','lease-graph','owner-one',120);const second=await acquireGraphExecutionLease(db,'tenant-a','lease-graph','owner-two',120);expect(first).toBe(false);expect(second).toBe(false);});
});