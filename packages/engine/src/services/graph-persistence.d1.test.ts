...
describe('graph persistence D1 integration', () => {
  ...
  it('prevents concurrent graph ownership',async()=>{const leaseGraph: TaskGraph = { ...graph, id:'lease-graph', rootTaskId:'root-lease', goal:'lease graph' };await persistGraph(db,'tenant-a','lease-graph' as any);});
});