...
export function parseRepairPatchDocument(value: string): Array<{ path: string; content: string }> {
  const trimmed = value.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, ''); let parsed: unknown;
  ...
  const result = files.map(entry => { if (!entry || typeof entry !== 'object') throw new Error('Repair patch contains an invalid file'); const file = entry as { path?: unknown; content?: unknown }; if (typeof file.path !== 'string' || !file.path.trim() || typeof file.content !== 'string') throw new Error('Repair patch file must contain path and content'); return { path: file.path, content: file.content }; });
...
async function versionPatch(env: HonoEnv['Bindings'], tenantId: string, projectId: string, files: Array<{ path: string; content: string }>): Promise<ProjectPatchInput[]> {
  ...
  for (const file of files) { try { const current = await readProjectFile(env, tenantId, projectId, file.path); versioned.push({ ...file, expectedVersion: current.version }); } catch (error) { if (!(error instanceof Error) || error.message !== 'Project file not found') throw error; versioned.push(file); } }
  return versioned;
}
export async function buildRepairProposal(env: HonoEnv['Bindings'], tenantId: string, projectId: string, node: TaskNode): Promise<RepairProposal> {
  const attempts = node.repairAttempts || 0; if (attempts >= MAX_GRAPH_REPAIR_ATTEMPTS) return { attempted: false, exhausted: true, reason: 'Maximum repair attempts reached' };
  const failure = node.verification?.reason || node.error || 'Runtime verification failed';
  const instruction = ['Diagnose and repair the project so the failed execution objective can pass.', `Node: ${node.title}`, `Requested operation: ${node.prompt}`, `Failure: ${failure}`, `Repair attempt: ${attempts + 1} of ${MAX_GRAPH_REPAIR_ATTEMPTS}`, 'Return ONLY JSON in this exact shape: {"files":[{"path":"relative/path","content":"complete file contents"}]}.', 'Return complete replacement file contents, not diffs. Change only files necessary to address the observed failure.', 'Do not claim tests passed unless they were actually executed.'].join('\n');
