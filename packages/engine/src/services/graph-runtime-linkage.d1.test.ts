...
describe('graph runtime linkage D1 integration', () => {
  ...
  beforeAll(async () => {
    ...
    for (const migration of ['0002_task_graph_persistence.sql','0003_graph_durable_execution.sql','0010_graph_node_approvals.sql','0011_runtime_job_linkage.sql','0012_graph_node_tool_persistence.sql','0013_graph_verification_repair.sql']) await execSqlFile(db, resolve(root, `migrations/${migration}`));
    ...
    await db.prepare('INSERT INTO projects (id,tenant_id,name) VALUES (?,?,?)').bind('project-1','runtime-tenant','Runtime Project').run();
  });
  afterAll(async () => { await dispose?.(); });

  it('round-trips runtime, approval, verification, and project-tool state without losing durable execution metadata', async () => {
    const graph: TaskGraph = {
      ...
      nodes: [{ id: 'execute', title: 'Run tests', prompt: 'npm test', domain: 'code', complexity: 1, expectedFormat: 'text', recommendedTier: 1,
        ...
        verification: { passed: false, reason: 'Runtime execution is not terminal', checkedAt: new Date().toISOString() }, repairAttempts: 2 }]
    };
    ...
    expect(persisted?.nodes[0].verification?.passed).toBe(false);
    expect(persisted?.nodes[0].repairAttempts).toBe(2);
  });
});
