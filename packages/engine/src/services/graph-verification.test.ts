...
describe('graph runtime verification', () => {
  ...
  it('supports natural minimum result criteria', () => {
    const result = verifyRuntimeResult({ jobId: 'job-4', status: 'succeeded', exitCode: 0, output: 'a\nb\nc' }, 'at least 3 results');
    expect(result.passed).toBe(true);
  });
  it('fails when minimum result criteria are not met', () => {
    const result = verifyRuntimeResult({ jobId: 'job-5', status: 'succeeded', exitCode: 0, output: 'a\nb' }, 'at least 3 results');
    expect(result.passed).toBe(false);
  });
  it('fails unsupported criteria closed', () => {
    const result = verifyRuntimeResult({ jobId: 'job-6', status: 'succeeded', exitCode: 0, output: 'done' }, '20 products must exist in database');
    expect(result.passed).toBe(false);
