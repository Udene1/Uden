...
import { assertRuntimeCapabilityPolicy } from './capability-policy';
...
export async function readProjectFile(env: Env, tenantId: string, projectId: string, path: string): Promise<ProjectFile> { const normalized = safePath(path); const row = await env.DB.prepare('SELECT path,content,version,content_sha256 FROM project_files WHERE tenant_id=? AND project_id=? AND path=?').bind(tenantId, projectId, normalized).first<{ path: string; content: string; version: number; content_sha256: string }>(); if (!row) throw new Error('Project file not found'); if (row.content.length > MAX_READ_BYTES) throw new Error('Project file exceeds read limit'); return { path: row.path, content: row.content, version: row.version, contentSha256: row.content_sha256 }; }
export async function projectTree(env: Env, tenantId: string, projectId: string): Promise<ProjectContextFile[]> { const files = await listProjectFiles(env, tenantId, projectId); return files.map(file => ({ path: file.path, version: file.version, contentSha256: file.contentSha256, size: file.content.length })); }
